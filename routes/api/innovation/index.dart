...
Future<Response> onGet(RequestContext con) async {
  InnovationRepository innovationRepo = con.read<InnovationRepository>();

  //AUTHORIZATION
  User user = con.read<User>();
  if(!user.isContainOne(["SUPERADMIN","ADMIN","ADMIN_INVENTORIES","PEGAWAI","GET_INNOVATIONS"])){
  ...
  try{
    List<Innovation> list_object = await innovationRepo.readAll();
...
Future<Response> onPost(RequestContext ctx) async {
  ...
  try{
    ...
    Innovation innovation = Innovation.fromJson(jsonMap as Map<String,dynamic>);
    var result = await innovationRepo.create(innovation);
