...
Future<Response> onGet(RequestContext ctx, String uuid) async {
  ...
  try{
    ...
    if(photo == null){
      ...
      if(photo.photo3_loc == null || photo.photo3_loc!.trim().isEmpty) {
        return RespHelper.badRequest(message: "There is no Data");
      }
      File photoFile = File(photo.photo3_loc!.trim());
      ...
      return Response.bytes(
        body: bytesPhoto,
        headers: {
          "Content-Disposition":"attachment;filename=\"${photo!.uuid!}_photo3${photo!.photo3_ext!}\""
        }
      );
    }
  } catch(e){
...
Future<Response> onPost(RequestContext ctx, String uuid) async {
  ...
  try {
    ...
    if(Platform.isWindows){
      ...
      File file  = File("${windows_dir}\\${photo_uuid}_photo3${extension}");
      ...
      File file  = File("${linux_dir}/${photo_uuid}_photo3${extension}");
      await file.writeAsBytes(file_bytes);
      location = file.absolute.path;
    } else{
    ...
    if(photo == null) {
      var result = await photoRepo.create(
        PenugasanPhoto(
            kmp_uuid: uuid,
            photo3_loc: location,
            photo3_ext: extension,
            last_updated: ""
          )
      );
      return Response.json(body: result);
    } else {
      photo.photo3_loc = location;
      photo.photo3_ext = extension;
