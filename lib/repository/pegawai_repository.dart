...
class PegawaiRepository extends MyRepository<Pegawai>{
  ...
  Future<Pegawai> update(dynamic pegawaiUuid,Pegawai pegawai) async{
    return this.connection.connectionPool.runTx<Pegawai>((tx) async {
      ...
      var result = await tx.execute(r"UPDATE pegawai SET fullname = $1, fullname_with_title = $2, nickname = $3, date_of_birth = $4, city_of_birth = $5, nip = $6, old_nip = $7, age = $8, username = $9, status_pegawai = $10)' WHERE uuid = $11",
      parameters: (pegawai.toJson()..remove("uuid")).values.toList()..add(pegawaiUuid as String));
